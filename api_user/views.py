...
from api_user.serializer import LoginSerializer, UserSerializer, CategorySerializer, ProductSerializer, CommentSerializer, CategoryBodySerializer, CommentBodySerializer, LoginBodySerializer, ProductBodySerializer, UserBodySerializer, EachUserBodySerializer, OrderSerializer, OrderBodySerializer, CategoryEachSerializer, UserProductsSerializer, ProductBodyPutSerializer, ProductPutSerializer, UserPutSerializer
...
class EachUserView(APIView):
    ...
    @swagger_auto_schema(request_body=UserPutSerializer, operation_summary="Update a user by user id")
    def put(self, request, **kwargs):
        ...
        if kwargs.get('user_id') is None:
            ...
            update_serializer = UserPutSerializer(user_object, data=request.data)
            ...
            if update_serializer.is_valid():
                ...
                return Response({'update':'fail', 'error':"Invalid request"}, status=status.HTTP_404_NOT_FOUND)
...
class EachCategoryView(APIView):
    ...
    def get(self, request, **kwargs):
        if kwargs.get('category_id') is None:
            ...
            category_serializer = CategorySerializer(category_queryset, many=True)
            return Response({'count':category_queryset.count(), 'categorys':category_serializer.data}, status=status.HTTP_200_OK)
        elif kwargs.get('category_id') is 7:
            # pro = Product.objects.count() # [1, 2, 3]
            # all_product = []
            # for i in range(pro):
            #     product = Product.objects.filter(product_id=i)
            #     product_serializer = CategoryEachSevenSerializer(product, many=True)
            #     all_product.extend(product_serializer.data)
            # category_serializer = CategoryEachSevenSerializer(Product.objects.all())
...
class EachProductView(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        if kwargs.get('product_id') is None:
            ...
            product_serializer = ProductSerializer(object)
            return Response({'products':product_serializer.data}, status=status.HTTP_200_OK)

    @swagger_auto_schema(request_body=ProductBodyPutSerializer, operation_summary="Update a product by product ID")
    def put(self, request, **kwargs):
        """
            상품을 수정하는 API
            ---
        ...
        if kwargs.get('product_id') is None:
            ...
            uid = kwargs.get('product_id')
            object = Product.objects.get(product_id=uid)
            update_serializer = ProductPutSerializer(object, data=request.data)
            if update_serializer.is_valid():
                update_serializer.save()
                return Response({'update':'true', 'data':update_serializer.data}, status=status.HTTP_200_OK)
            else:
                return Response({'update':'false', 'error':"Invalid request"}, status=status.HTTP_404_NOT_FOUND)
...
class OrderView(APIView):
    ...
    def post(self, request):
        ...
        user_query = Product.objects.filter(product_id=product_id).values('user_id')
        if user_id != user_query.values():
            serializer = OrderSerializer(data=request.data)
            if serializer.is_valid():
                serializer.save()
                return Response({'success':'user_query', 'comments':serializer.data, 're':user_id}, status=status.HTTP_200_OK)
            else:
                return Response({'success':'false', 'error':serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
        else:
